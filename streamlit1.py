...
if st.button("Get Global Explanation"):
    ...
    # Vérifier les noms des caractéristiques
    st.write("Noms des caractéristiques attendus :", scaler.feature_names_in_)
    st.write("Noms des caractéristiques fournis :", sample_data.columns)
    
    # Traiter les valeurs manquantes
    sample_data.fillna(0, inplace=True)  # Remplir avec 0 (ou utilisez une autre méthode)

    # Vérifier si sample_data n'est pas vide avant la transformation
    if not sample_data.empty:
        ...
        st.warning("Les données de l'échantillon ne sont pas disponibles.")
